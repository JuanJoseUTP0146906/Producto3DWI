<?php
session_start();
// Asegúrate de que `$_SESSION['email']` y `$_SESSION['is_logged_in']` estén establecidos en tu lógica de login
$email = isset($_SESSION['Usuario']) ? $_SESSION['Usuario'] : '';
$is_logged_in = isset($_SESSION['Usuario']) ? $_SESSION['Usuario'] : false;
$is_admin = strpos($email, '@automarket.com') !== false;
?>
...
<body>
            ...
            <div class="bottom-content">
                <?php if ($is_logged_in): ?>
                    ...
                    <li class="nav-link">
                        <a href="#">
                            <i class='bx bx-user icon'></i>
                            <span class="text nav-text"><?php echo htmlspecialchars($email); ?></span>
                        </a>
                    </li>
                <?php else: ?>
                    <li class="nav-link">
    ...
    <!-- Header -->
    <header class="bg-dark py-5" style="background-image: url('assets/img/fondo_home_page2.jpg'); background-size: cover; background-position: center;">
